{
  ...
  outputs =
    { self, ... }:
    {
      ...
      # Add after sops-nix when using sops
      nixosModules.qnixSopsIntegration = import ./modules/core/sops/nixos/sops-integration.nix;

      # Add after impermanence when using impermanence
      nixosModules.qnixImpermanenceIntegration = import ./modules/core/impermanence/nixos/impermanence-integration.nix;

      # Home Manager module loader
      homeManagerModules.qnix = import ./loader/home.nix;

      # Add after ags HM module when using AGS
      homeManagerModules.qnixAgsIntegration = import ./modules/desktop/hyprdesktop/home/ags/module.nix;

      # Add after noctalia HM module when using noctalia-shell
      homeManagerModules.qnixNoctaliaIntegration = import ./modules/desktop/hyprdesktop/home/noctalia/module.nix;
            ...
            let
              ...
              collectFromCategory =
                ...
                let
                  ...
                  optionPaths = builtins.map (
                    name: modulesRoot + "/${category}/${name}/options/options.nix"
                  ) moduleNames;
                in
                optionPaths;
