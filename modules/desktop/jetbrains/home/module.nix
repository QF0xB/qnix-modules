...
let
  cfg = osConfig.qnix.desktop.jetbrains;
in
{
  config = lib.mkIf cfg.enable {
    home.packages = lib.concatLists [
      ...
      (lib.optionals cfg.dataspell.enable [
        pkgs.jetbrains.dataspell
      ])
      (lib.optionals cfg.idea.enable [
        pkgs.jetbrains.idea
      ])
      (lib.optionals cfg.phpstorm.enable [
        pkgs.jetbrains.phpstorm
      ])
      (lib.optionals cfg.pycharm.enable [
        pkgs.jetbrains.pycharm
      ])
      (lib.optionals cfg.rider.enable [
        pkgs.jetbrains.rider
      ])
      (lib.optionals cfg.ruby-mine.enable [
        pkgs.jetbrains.ruby-mine
      ])
      (lib.optionals cfg.rust-rover.enable [
