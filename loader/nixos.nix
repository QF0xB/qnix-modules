...
let
  ...
  collectOptions =
    ...
    let
      ...
      optionPaths = builtins.map (
        name: (toString ./../modules) + "/${category}/${name}/options/options.nix"
      ) moduleNames;
    in
    optionPaths;
  ...
  collectNixosModules =
    ...
    let
      ...
      modulePaths = builtins.map (
        name: (toString ./../modules) + "/${category}/${name}/nixos/module.nix"
      ) nixosModules;
    in
    modulePaths;
  ...
  qnixNixosModule =
    if builtins.pathExists ./../modules/qnix/nixos/module.nix then
      [ ./../modules/qnix/nixos/module.nix ]
    else
      [ ];
