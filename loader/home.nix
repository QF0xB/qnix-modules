...
let
  ...
  collectHomeModules = category:
    let
      catModules = moduleIndex.${category} or {};
      moduleNames = builtins.attrNames catModules;
      # Only include modules that have home = true
      homeModules = builtins.filter (name:
        let info = catModules.${name};
        in info.home or false
      ) moduleNames;
      modulePaths = builtins.map (name: 
        (toString ./../modules) + "/${category}/${name}/home/module.nix"
      ) homeModules;
    in
    modulePaths;
  ...
  qnixHomeModule = if builtins.pathExists ./../modules/qnix/home/module.nix
    then [ ./../modules/qnix/home/module.nix ]
    else [];
in
lib.traceSeqN 1
  ">>> [qnix/home] Categories: ${builtins.toString validCategories}"
  {
    # Import qnix options first, then other options, then modules
    imports = qnixOptions ++ allOptionModules ++ qnixHomeModule ++ allHomeModules;
  }
